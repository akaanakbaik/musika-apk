...
import '../widgets/share_song.dart';
...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _downloadSong(Song song) async {
    ...
    try {
      ...
      if (res['success'] == true && res['download_url'] != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Link unduhan: ${res['download_url']}'), behavior: SnackBarBehavior.floating, duration: const Duration(seconds: 5)),
        );
      }
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                ...
                ? GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Icon(Icons.timer, size: 12, color: AppTheme.primary),
                          const SizedBox(width: 3),
                          Text(timer.remainingFormatted, style: const TextStyle(fontSize: 10, color: AppTheme.primary, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  )
      ...
      body: _showQueue ? _buildQueue(player, isDark) : _buildPlayer(player, song, isDark),
    );
  }
      ...
      child: Column(
        children: [
          ...
          Row(children: [
            ...
            Expanded(child: Padding(
              ...
              child: SliderTheme(
                ...
                child: Slider(
                  value: player.duration > 0 ? (player.position / player.duration).clamp(0.0, 1.0) : 0,
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
            IconButton(
              icon: Icon(Icons.shuffle, size: 24, color: player.shuffle ? AppTheme.primary : Colors.grey[400]),
            ...
            Container(
              ...
              child: IconButton(
                icon: Icon(player.loading ? Icons.hourglass_top : player.playing ? Icons.pause : Icons.play_arrow, size: 40, color: Colors.black),
            ...
            IconButton(
              icon: Icon(player.repeatMode == RepeatMode.one ? Icons.repeat_one : Icons.repeat, size: 24,
                  color: player.repeatMode != RepeatMode.none ? AppTheme.primary : Colors.grey[400]),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            IconButton(
              icon: Icon(player.muted ? Icons.volume_off : Icons.volume_up, size: 20, color: Colors.grey[400]),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
            ...
            _actionButton(Icons.share_outlined, 'Bagikan', () {
    ...
    return Column(children: [
      ...
      Expanded(
        child: q.isEmpty
            ...
            : ListView.builder(
                itemCount: q.length.clamp(0, 20),
                itemBuilder: (_, i) {
                  final qSong = q[i];
                  final isCurrent = qSong.videoId == player.currentSong?.videoId;
                  return ListTile(
                    leading: Container(
                      width: 40, height: 40,
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(8),
                        image: qSong.thumbnail.isNotEmpty
                            ? DecorationImage(image: NetworkImage(qSong.thumbnail), fit: BoxFit.cover)
                            : null,
                    ...
                    subtitle: Text(qSong.artist, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12, color: Colors.grey[500])),
